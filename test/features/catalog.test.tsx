...
describe('<CatalogGrid>', () => {
  ...
  it('isolates every figure in dir="ltr" so the minus keeps its side', () => {
    ...
    expect(figures.length).toBe(catalog.vehicles.length * 3)
  ...
  it('marks a cost that rests on an estimated consumption figure', () => {
    // Every row reads "estimate" today (docs/ASSUMPTIONS.md section ה), so the
    // mark belongs on every card until importer data replaces them.
    expect(html.match(/צריכה משוערת/g)).toHaveLength(catalog.vehicles.length)
  })
